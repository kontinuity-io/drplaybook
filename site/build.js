...
function buildPatternsIndex() {
  const patternsDir = path.join(ROOT, 'patterns');
  if (!fs.existsSync(patternsDir)) return [];
  const patterns = [];
  for (const slug of fs.readdirSync(patternsDir)) {
    const metaPath = path.join(patternsDir, slug, 'pattern.json');
    if (fs.existsSync(metaPath)) {
      const meta = JSON.parse(fs.readFileSync(metaPath, 'utf8'));
      patterns.push({ ...meta, slug });
    }
  }
  return patterns.sort((a, b) => a.category.localeCompare(b.category) || a.title.localeCompare(b.title));
}
...
function main() {
  ...
  const patterns = buildPatternsIndex();

  const data = {
    generated: new Date().toISOString().slice(0, 10),
    stats: { totalChapters: chapters.length, completeChapters, progressChapters, totalLessons, completeLessons, totalPatterns: patterns.length },
    chapters,
    patterns,
  };
...
`;
  ...
  console.log(`  ${chapters.length} chapters  |  ${completeLessons}/${totalLessons} lessons complete  |  ${completeChapters} chapters done  |  ${patterns.length} patterns`);
}
